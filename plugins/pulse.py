...
class PulseAppVolume(object):
	...
	def _toggle_mute(self, context, sink_input_info):
		self._mute(context, sink_input_info, int(not sink_input_info.mute))
	...
	def _sink_input_info_callback(self, context, info, eol, userdata):
		...
		if info.contents.name == self.app_name:
			...
			if self.toggle_mute:
				self._toggle_mute(context, info.contents)
			if self.mute is not None:
				self._mute(context, info.contents, int(self.mute))
	...
	def _main(self):
		...
		context = _lib.pa_context_new(mainloop_api, "wmii") # Alt: _with_proplist
